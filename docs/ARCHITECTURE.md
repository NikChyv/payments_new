...
| `notify_failures` | неудачные отправки уведомлений; читать через `notify_failures_recent`, снаружи закрыта |
...
**Утренний список**
`pg_cron` в 05:30 UTC (8:30 Минск) → `send_daily_reminder()` → по письму на
каждого из `daily_reminder_targets()` (`staff.telegram_id`) → `net.http_post` →
Telegram. Текст собирает `daily_reminder_message(staff_id)`: платежи на сегодня
со ссылками на файлы плюс **собственные** личные задачи получателя. Пока
`staff.telegram_id` не заполнен — запасной путь по зашитым chat_id, без личных
задач вовсе.

**Уведомление не дошло**
Любая осечка отправки (Telegram ответил не 200, не отозвался, файл не
скачался) → запись в `notify_failures`. `health.yml` каждые 15 минут читает
`notify_failures_recent(24)` — только счётчик, публичным ключом — и краснеет,
если за сутки было хоть одно.
...
supabase/
  ...
  tests/                 pgTAP: 286 тестов в 19 файлах
