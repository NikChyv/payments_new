...
Один чат может быть привязан к **нескольким фирмам** (человек ведёт две
компании): `clients.telegram_id` не уникален, `/start` привязки складывает.
Уведомления от этого не страдают — они идут от заявки к фирме. Диалог создания
заявки, наоборот, требует ровно одной фирмы, поэтому при нескольких бот
отправляет человека на персональную ссылку нужной компании.
...
`20260826000003_storage_limits.sql` и проверяются тестом `storage_limits_test.sql`.
Менять их кликами в панели не надо — правка потеряется при следующем `db push`.
